...
patientIRAQ <- read.delim("D:/Users/iraq-epidem/06. Data/NCD Data Tool/Rscript/PatientKHQ.txt",stringsAsFactors=FALSE)
visitIRAQ <- read.delim("D:/Users/iraq-epidem/06. Data/NCD Data Tool/Rscript/VisitKHQ.txt",stringsAsFactors=FALSE)

# 4) Indicate the time range of your reporting period. Then RUN these 3 lines:
Upperlimitmonth <- "2017-10-29" # 1 day after the last day in the reporting period
Lowerlimitmonth <- "2017-09-30" # 1 day before the first day in the reporting period
Workingdaysmonth <- 11 # number of clinic working days in reporting period (calendar days minus days off)
...
# Last visit of new patients (not always where they were enrolled) 
...
# First visit of new patients  => Replaced lines above to give admission patterns by clinic instead of clinic where new patients were last seen.
Total_visitP_new <- visitP[visitP$Admission.date < Upperlimitmonth & visitP$Admission.date > Lowerlimitmonth,] # all visits of new patients
First_visitP_new <- setkey(setDT(Total_visitP_new), Internal.ID)[, .SD[which.min(Date.of.visit)], Internal.ID]

# Number of new patients this reporting period by clinic/hv of their last visit 
# With change above, this will become the clinic where new patients are first seen. 
Total_new_Alwand <- First_visitP_new[First_visitP_new$Clinic == "Alwand 1 PHCC",]
Total_new_Alwand_N <- NROW(Total_new_Alwand); perc_Total_new_Alwand <-Total_new_Alwand_N/Total_new_enrolled_n
Total_new_AlwandHV <- First_visitP_new[First_visitP_new$Clinic == "Home Visit(Alwand Camps)",]
Total_new_AlwandHV_N <- NROW(Total_new_AlwandHV); perc_Total_new_AlwandHV <-Total_new_AlwandHV_N/Total_new_enrolled_n
Total_new_Jalawla <- First_visitP_new[First_visitP_new$Clinic == "Jalawla PHCC",]
Total_new_Jalawla_N <- NROW(Total_new_Jalawla); perc_Total_new_Jalawla <-Total_new_Jalawla_N/Total_new_enrolled_n
Total_new_Sadiya <- First_visitP_new[First_visitP_new$Clinic == "Sadiya PHCC",]
Total_new_Sadiya_N <- NROW(Total_new_Sadiya); perc_Total_new_Sadiya <-Total_new_Sadiya_N/Total_new_enrolled_n

New_enrolled<- data.frame()
New_enrolled_Alwand = c(Total_new_Alwand_N,perc_Total_new_Alwand); New_enrolled = rbind(New_enrolled,New_enrolled_Alwand)
New_enrolled_AlwandHV = c(Total_new_AlwandHV_N,perc_Total_new_AlwandHV); New_enrolled = rbind(New_enrolled,New_enrolled_AlwandHV)
New_enrolled_Jalawla = c(Total_new_Jalawla_N,perc_Total_new_Jalawla); New_enrolled = rbind(New_enrolled,New_enrolled_Jalawla)
New_enrolled_Sadiya = c(Total_new_Sadiya_N,perc_Total_new_Sadiya); New_enrolled = rbind(New_enrolled,New_enrolled_Sadiya)
colnames(New_enrolled) <- c("Freq.x","Freq.y")
New_enrolled$Var1<-c( "New patients this period - Admitted in Alwand 1 PHCC", "New patients this period - Admitted in Alwand Camp HVs","New patients this period - Admitted in Jalawla PHCC", "New patients this period - Admitted in Sadiya PHCC")
New_enrolled<-New_enrolled[,c("Var1","Freq.x","Freq.y")]

# Number of consultations this reporting period by clinic/hv. 
...
# Number consultations this reporting period by clinic/hv by type of practitioner (nurse or doctor) 
...
# Number of referrals this reporting period by type of referral (destination)
...
# Proposed replacement for lines 117-144 (acute and chronic referrals instead of by each individual department)
#Total_new_Referral <- Total_new_consults[Total_new_consults$Referral == "Yes",]; New_referral<-NROW(Total_new_Referral)
...
# Number of referrals this period by referring clinic site => Was omitted from output before?
Total_new_referral_clin <- Total_new_consults[Total_new_consults$Referral == "Yes",]
Total_new_referral_clin_N <- table(Total_new_referral_clin$Clinic); Total_new_referral_clin_N_df <- as.data.frame(Total_new_referral_clin_N)
Total_new_referral_clin_N_per <- prop.table(Total_new_referral_clin_N); Total_new_referral_clin_N_per_df <- as.data.frame(Total_new_referral_clin_N_per)
Total_new_referral_clinic <- merge(Total_new_referral_clin_N_df, all.x = TRUE, Total_new_referral_clin_N_per_df, c("Var1"))
Total_new_referral_clinic$Var1 <- c(paste("Referrals this period",Total_new_referral_clinic$Var1,sep=" - "))
...
# Propose replacing lines 177-183 with the following.
...
# Status (host/IDP/refugee) of actively cohort  
Total_status_N<-table(Active_last_visitP$Status); Total_status_N_df<-as.data.frame(Total_status_N)
Total_status_per<-prop.table(Total_status_N); Total_status_per_df<-as.data.frame(Total_status_per)
(Total_status <- merge(Total_status_N_df, all.x = TRUE, Total_status_per_df, c("Var1")))
Total_status$Var1 <- c(paste("Status of patients active at end of period",Total_status$Var1,sep=" - "))
...
MMR.IRAQ <- rbind(New_enrolled, Total_new_consults_clinic, Total_new_consults_Doc_clinic, Total_new_consults_Nurse_clinic,Total_Referral, Total_new_referral_clinic, Total_enrolled_active, Total_exit_new, Total_gender, Age, Total_status, Total_Morbidities)
write.csv(MMR.IRAQ, file = "MMR.IRAQ.KHQv2b.csv")
...
# distibution of time between last visit and upper limit of reporting month
Total_active_visitP$diff_in_days<- difftime(Total_active_visitP$Date.of.visit ,Upperlimitmonth , units = c("days"))
Total_active_visitP$diff_in_days # to get the difference in time between last visit and upperlimit in days per active patient

# Generates a text file with a list of all defaulting patients who have not yet been exited from the cohort at the start of the current reporting period.
Total_exit_recs <- Total_exit_recs[Total_exit_recs$Exit == "",]
write.csv(Total_exit_recs, file = "KHQ_NCD_Defaulter_Tracing_List.csv")
